...
Template.editCustomer.events({
    'submit #form-customer': function(event){
        ...
        let new_customer = instance.state.get('customer');

        let userObj = {firstName: firstName, lastName: lastName, email: email};//, role: role
        ...
        if(instance.getUserId()) {
            //let userTemp = instance.state.get('user');
    ...
    {
        //let user = instance.state.get('user');
